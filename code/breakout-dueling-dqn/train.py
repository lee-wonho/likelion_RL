...
from tensorflow.keras.layers import Conv2D, Dense, Flatten , Add, Subtract, Lambda
...
class Dueling_DQN(tf.keras.Model):
    def __init__(self, action_size, state_size):
        ...
        self.fc_value = Dense(1,activation ='relu')
        self.fc2 = Dense(512,activation='relu')
        self.fc_adv = Dense(action_size)
        self.adv_mean = Lambda(lambda x: tf.keras.backend.mean(x,axis=1))
        self.a_sub = Subtract()
        self.add = Add()
    ...
    def call(self, x):
        ...
        a = self.fc2(x)
        a = self.fc_adv(a)
        a_mean = self.adv_mean(a)
        a = self.a_sub([a, a_mean])
        q = self.add([v, a])
        return q
...
class Dueling_DQNAgent:
    def __init__(self, action_size, state_size=(84, 84, 4)):
        ...
        # 모델과 타깃 모델 생성
        self.model = Dueling_DQN(action_size, state_size)
        self.target_model = Dueling_DQN(action_size, state_size)
        ...
        self.writer = tf.summary.create_file_writer('summary/breakout-dueling-dqn')
    ...
    def train_model(self):
        ...
        with tf.GradientTape() as tape:
            ...
            #다음 상태에 대한 모델의 큐함수
            next_predicts = self.model(next_history)
            next_one_hot_action = tf.one_hot(np.argmax(next_predicts,axis=1),self.action_size)


            # 벨만 최적 방정식을 구성하기 위한 타깃과 큐함수의 값 계산
            #DDQN 코드 수정
            target_q = tf.reduce_sum(next_one_hot_action*target_predicts,axis=1)
            targets = rewards + (1 - dones) * self.discount_factor * target_q
...
if __name__ == "__main__":
    ...
    agent = Dueling_DQNAgent(action_size=3)
